...
class SocialFish(HackingTool):
    ...
    INSTALL_COMMANDS = [
    # 1. Install necessary system packages (git and python3-venv) and clone the repository.
    "sudo apt-get update && sudo apt-get install git python3 python3-venv -y && git clone https://github.com/UndeadSec/SocialFish.git",
    
    # 2. Change directory, create and activate the virtual environment.
    "cd SocialFish && python3 -m venv .venv && source .venv/bin/activate",
    
    # 3. Install requirements using the virtual environment's pip (no sudo needed here).
    "cd SocialFish && source .venv/bin/activate && pip install -r requirements.txt"
    ]
